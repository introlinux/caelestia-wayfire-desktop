...
Searcher {
    ...
    function launch(entry: DesktopEntry): void {
        ...
        const cmd = entry.runInTerminal
            ? ["app2unit", "--", ...Config.general.apps.terminal, `${Quickshell.shellDir}/assets/wrap_term_launch.sh`, ...entry.command]
            : ["app2unit", "--", ...entry.command];

        console.log("[Apps] launch cmd:", JSON.stringify(cmd), "workDir:", entry.workingDirectory)
