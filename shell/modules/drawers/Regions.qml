...
Region {
    ...
    R {
        panel: root.panels.miniapps
        y: root.win.height - height
        height: panel.height * (1 - root.panels.miniapps.offsetScale) + root.borderThickness
    }
    ...
    R {
        panel: root.panels.windowControls
        y: 0
        height: panel.height * (1 - root.panels.windowControls.offsetScale) + root.borderThickness
    }
    ...
    R {
        panel: root.panels.appgrid
        width: panel.width * (1 - panel.offsetScale)
        height: panel.height * (1 - panel.offsetScale)
    }
