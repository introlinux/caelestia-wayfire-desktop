    ...
    RowLayout {
        ...
        ColumnLayout {
            ...
            WrappedLoader {
                ...
                sourceComponent: FetchText {
                    text: `BATT: ${[UPowerDeviceState.Charging, UPowerDeviceState.FullyCharged, UPowerDeviceState.PendingCharge].includes(UPower.displayDevice.state) ? "(+) " : ""}${Math.round(Math.min(UPower.displayDevice.percentage, 1.0) * 100)}%`
                }
            }
        }
    }
