...
/**
 * Server side decorations that mimic the GTK/Adwaita header bar, so that
 * Qt, old GTK and other toolkits which do not draw their own decorations
 * blend in with the rest of the desktop.
 *
 * Forked from Wayfire's built-in `decoration` plugin; it must be used
...
class wayfire_gtkdecor : public wf::plugin_interface_t
{
        ...
        [=] (wf::txn::new_transaction_signal *ev)
    {
        ...
        for (const auto& obj : ev->tx->get_objects())
        {
            if (auto toplevel = std::dynamic_pointer_cast<wf::toplevel_t>(obj))
            {
                ...
                if (auto deco = toplevel->get_data<wf::gtk_decorator_t>())
                {
  ...
  public:
    ...
    void adjust_new_decorations(wayfire_toplevel_view view)
    {
        ...
        toplevel->store_data(std::make_unique<wf::gtk_decorator_t>(view));
        auto deco     = toplevel->get_data<wf::gtk_decorator_t>();
    ...
    {
        view->toplevel()->erase_data<wf::gtk_decorator_t>();
    ...
    {
        return toplevel->has_data<wf::gtk_decorator_t>();
    }
