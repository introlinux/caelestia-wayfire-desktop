...
decoration_layout_t::decoration_layout_t(const decoration_theme_t& th,
    ...
    button_size(th.get_button_size()),
    button_padding(th.get_button_padding()),
    button_spacing(th.get_button_spacing()),
    theme(th),
    damage_callback(callback)
{}

int decoration_layout_t::get_reserved_width() const
{
...
wf::geometry_t decoration_layout_t::create_buttons(int width, int)
{
    ...
    if (buttons.empty())
    {
        this->reserved_width = border_size + button_padding;

        return {width - reserved_width, border_size, 0, titlebar_size};
    }

    /* The buttons are vertically centred in the whole title bar band, the top
     * border included - that is how a GTK header bar lines them up. */
    const int band_height = titlebar_size + border_size;
    const int button_y    = (band_height - button_size) / 2;

    int button_x = width - border_size - button_padding - button_size;
    for (auto it = buttons.rbegin(); it != buttons.rend(); ++it)
    {
        this->layout_areas.push_back(std::make_unique<decoration_area_t>(
            wf::geometry_t{button_x, button_y, button_size, button_size},
            damage_callback, theme));
        this->layout_areas.back()->as_button().set_button_type(*it);
        button_x -= button_size + button_spacing;
    }

    /* Where the leftmost button starts */
    const int zone_x = button_x + button_size + button_spacing;
    this->reserved_width = width - zone_x + button_spacing;

    return {zone_x, border_size, width - zone_x, titlebar_size};
}
