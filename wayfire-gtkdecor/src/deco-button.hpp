...
class button_t
{
  public:
    ...
    /** Set the type of the button, which decides the icon drawn on it. */
     ...
     * @param active Whether the window owning the button is focused
     */
    void render(const scene::render_instruction_t& data, wf::geometry_t geometry, bool active);
  ...
  private:
    ...
    /* Parameters the current texture was baked with */
    ...
    void update_texture(double size, double scale, bool active);
};
}
}
